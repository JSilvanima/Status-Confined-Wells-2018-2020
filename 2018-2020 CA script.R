...
# The function determine which wells are no longer included in the target 
#  population by comparing the primary key for each well from the most 
#  recent well list frame to those found in the data pull for the three 
#  year period. The resultant data frame is named well_removals.
#
# Take a look at well_removals next.
...
# Create new data frame by cloning SiteEvaluations and name it CA.SITES.
...
# Of the 15,339 confined wells in the sample frame, 98.0 % are estimated 
#  to be in the target population, i.e., 15,040 wells. Also, only 29.2 % 
...
# 29.7 % of the target population of wells could be sampled, i.e., 
#  4,473 wells CI(3,882 - 5,064) wells.
...
FDEPgetdata::getdata_results("'CA18','CA19','CA20'")
...
# Note that we have BLANK and PRIMARY data and matrix types. A total of 363 
#  wells sampled for water quality. 


# Identify the samples which were analyzed but do not match the wells in
#  the SiteEvaluation data frame and document them in a comment below.

Removed_Results <- sqldf("Select * from Results
          where FK_RANDOM_SAMPLE_LOCATION
                        not in (select PK_RANDOM_SAMPLE_LOCATION
                          from SiteEvaluations)")
View(Removed_Results)

# Note Z2-CA-12021B, Z2-CA-12037B, Z3-CA-12025B, Z3-CA-13013B, Z3-CA-13038B, 
#  Z3-CA-13042B, Z4-CA-12040B, Z4-CA-14009B, Z5-CA-14004B, Z6-CA-13013B, and 
#  Z6-CA-13023B were either inappropriately sampled or expired holding times. 
#  Sample Z6-CA-12020 was removed because it no longer is in the target 
#  population.


# Water Quality Analyses
# Water quality analyses are based on 351 wells from the results file and must be merged with design information.
...
write.csv(Water_Quality_Cat, "CA18_CA19_CA20_Cat.csv")
write.csv(Water_quality_Cont$CDF, file = 'CA18_CA19_CA20_Cont_EstCDF.csv')
write.csv(Water_quality_Cont$Pct, file = 'CA18_CA19_CA20_Cont_EstPCT.csv')
