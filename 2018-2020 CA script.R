...
#  dataframe for FDEP Status confined aquifers sites evaluated 2018 - 2020.
#  Be sure to enclose in double and single quotes.
#
# Water resource acronyms are CA = Confined Aquifers, UA = Unconfined Aquifers.


FDEPgetdata::getdata_aq_exclusions("'CA18','CA19','CA20'")

# Function getdata_aq_exclusions creates a dataframe names 'Exclusions' from the 
...
#  well list frame to those found in the data pull for the three year period.

# Create data frame with wells to be removed.

channel <- odbcConnect("GWIS_ADMIN",uid="GWIS_ADMIN",pwd=rstudioapi::askForPassword("GWIS Password"))

well_removals<-sqlQuery(channel,"select * from site_evaluations
where fk_well_listframe_id not in (select distinct(id) from well_listframe_2020)
and substr(site_evaluations.fk_project,3,4) in ('CA18','CA19','CA20')
...
# A simplier way to do the same as the above by using the sqldf function
